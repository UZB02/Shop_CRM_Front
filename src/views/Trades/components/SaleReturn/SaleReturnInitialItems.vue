<template>
  <div class="space-y-3">
    <h3 class="text-[11px] font-black text-slate-400 dark:text-slate-500 tracking-[0.2em] ml-1">Savdodagi mahsulotlar</h3>
      ...
      <div v-for="item in initialSale.items" :key="item.id" 
        ...
        <div class="flex flex-col">
          ...
          <div class="flex items-center flex-wrap gap-x-2 gap-y-1 mt-1">
            <span class="text-[11px] text-slate-500 font-bold tracking-widest">Sotilgan: {{ parseFloat(item.quantity) }}</span>
            <template v-if="parseFloat(item.returned_qty || 0) > 0">
              <span class="w-1 h-1 rounded-full bg-slate-300"></span>
              <span class="text-[11px] text-amber-500 font-bold tracking-widest">Qaytarilgan: {{ parseFloat(item.returned_qty) }}</span>
            </template>
            <span class="w-1 h-1 rounded-full bg-slate-300"></span>
            <span class="text-[11px] text-emerald-500 font-bold tracking-widest">Qolgan: {{ parseFloat(item.net_qty || (parseFloat(item.quantity) - parseFloat(item.returned_qty || 0))) }}</span>
            <span class="w-1 h-1 rounded-full bg-slate-300"></span>
            <span class="text-[11px] text-slate-400 font-medium tracking-tight">{{ formatCurrency(item.unit_price) }}</span>
          </div>
        </div>
        <button 
          @click="$emit('add', item)"
          ...
          class="h-7 px-3 rounded-lg text-[11px] font-black tracking-widest transition-all
