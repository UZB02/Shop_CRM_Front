        ...
        <div class="flex flex-col">
          ...
          <div class="flex items-center flex-wrap gap-x-2 gap-y-1 mt-1">
            ...
            <template v-if="parseFloat(item.returned_qty || 0) > 0">
              ...
              <span class="text-[9px] text-amber-500 font-bold uppercase tracking-widest">Qaytarilgan: {{ parseFloat(item.returned_qty) }}</span>
            </template>
            <span class="w-1 h-1 rounded-full bg-slate-300"></span>
            <span class="text-[9px] text-emerald-500 font-bold uppercase tracking-widest">Qolgan: {{ parseFloat(item.net_qty || (parseFloat(item.quantity) - parseFloat(item.returned_qty || 0))) }}</span>
            <span class="w-1 h-1 rounded-full bg-slate-300"></span>
