<template>
  <div class="mt-2 flex-grow flex flex-col min-h-0">
    <div class="flex items-center justify-between mb-3 shrink-0">
      <h3 class="text-[11px] font-black text-slate-400 dark:text-slate-500 tracking-[0.2em] ml-1">Qaytariladigan mahsulotlar</h3>
      <span class="text-[11px] font-bold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-500/10 px-2 py-1 rounded-md border border-rose-100 dark:border-rose-500/10 leading-none">{{ items.length }} xil</span>
    </div>
    ...
    <div class="space-y-2.5 flex-grow overflow-y-auto pr-1 custom-scrollbar">
      <div v-if="!items.length" class="h-full flex flex-col items-center justify-center opacity-40 py-16">
        ...
        <p class="text-[13px] font-bold dark:text-slate-400">Hech narsa tanlanmagan</p>
      </div>
      ...
      <div v-for="(item, idx) in items" :key="idx" 
        ...
        <div class="flex-grow min-w-0">
          ...
          <p class="text-[11px] text-slate-500 dark:text-slate-400 font-medium mt-0.5">Birlik: {{ formatCurrency(item.unit_price) }}</p>
        </div>
        ...
        <div class="w-20 text-right shrink-0">
          <span class="text-[14px] font-black text-slate-900 dark:text-white leading-none">{{ formatCurrency(item.unit_price * item.quantity) }}</span>
        </div>
