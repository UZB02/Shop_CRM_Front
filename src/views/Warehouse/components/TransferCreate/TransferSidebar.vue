  ...
  <div class="w-full lg:w-72 flex flex-col gap-3 sm:gap-4 shrink-0 overflow-y-auto no-scrollbar pb-10">
    ...
    <div class="bg-white dark:bg-[#1e293b]/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-2xl p-3.5 sm:p-4 shadow-sm space-y-3 sm:space-y-3.5">
      <div class="flex items-center justify-between">
        <h3 class="text-[10px] sm:text-[11px] font-black text-slate-400 tracking-[0.1em] uppercase opacity-70">{{ $t('warehouse.transfer.receiver') }}</h3>
        <span class="text-[9px] sm:text-[10px] font-bold text-rose-500 uppercase tracking-tighter">* {{ $t('warehouse.transfer.required') }}</span>
      </div>
      ...
      <div class="grid grid-cols-2 gap-1 p-1 bg-slate-100 dark:bg-slate-900 rounded-xl border border-slate-200/20 dark:border-slate-800/50 shadow-inner">
        <button
          ...
          class="flex items-center justify-center gap-1.5 py-2 sm:py-1.5 rounded-lg text-[11px] sm:text-[12px] font-black tracking-widest transition-all uppercase"
      ...
      <div class="relative group">
        ...
        <select
          ...
          class="w-full h-10 pl-10 pr-8 text-[13px] sm:text-[14px] font-bold rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 text-slate-800 dark:text-white focus:outline-none focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 transition-all appearance-none"
        >
        ...
        <select
          ...
          class="w-full h-10 pl-10 pr-8 text-[13px] sm:text-[14px] font-bold rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 text-slate-800 dark:text-white focus:outline-none focus:ring-4 focus:ring-amber-500/5 focus:border-amber-500 transition-all appearance-none"
        >
        ...
        <i class="pi pi-chevron-down text-[8px] text-slate-400 absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none group-focus-within:rotate-180 transition-transform"></i>
      </div>
      ...
      <Transition name="fade">
        ...
        >
          ...
          <div class="flex flex-col min-w-0">
            <span class="text-[9px] font-black text-emerald-500 tracking-widest uppercase opacity-80 leading-none mb-1">{{ $t('warehouse.transfer.receiver') }}</span>
            <span class="text-[12px] sm:text-[13px] font-black text-emerald-700 dark:text-emerald-400 truncate">{{ selectedDestinationName }}</span>
          </div>
        </div>
      </Transition>
    </div>
    ...
    <!-- Summary Card -->
    <div class="bg-white dark:bg-[#1e293b]/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-2xl p-3.5 sm:p-4 shadow-sm space-y-3 sm:space-y-3.5">
      <h3 class="text-[10px] sm:text-[11px] font-black text-slate-400 tracking-[0.1em] uppercase opacity-70">{{ $t('warehouse.transfer.summary') }}</h3>
      ...
      <div class="space-y-2 sm:space-y-2.5">
        <div class="flex items-center justify-between p-2.5 sm:p-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800/50">
          <div class="flex flex-col min-w-0">
            <span class="text-[8px] sm:text-[9px] font-black text-slate-400 tracking-widest uppercase mb-0.5 opacity-60 leading-none truncate">{{ $t('warehouse.transfer.source') }}</span>
            <span class="text-[11px] sm:text-[12px] font-black text-slate-700 dark:text-white truncate max-w-[80px] sm:max-w-[100px]">{{ sourceName || '—' }}</span>
          </div>
          <i class="pi pi-arrow-right text-[10px] text-slate-300 shrink-0"></i>
          <div class="flex flex-col text-right min-w-0">
            <span class="text-[8px] sm:text-[9px] font-black text-slate-400 tracking-widest uppercase mb-0.5 opacity-60 leading-none truncate">{{ $t('warehouse.transfer.destination') }}</span>
            <span class="text-[11px] sm:text-[12px] font-black text-slate-700 dark:text-white truncate max-w-[80px] sm:max-w-[100px]">{{ selectedDestinationName || '—' }}</span>
          </div>
        </div>
      ...
      <div v-if="!isValid" class="p-3 rounded-xl bg-rose-50 dark:bg-rose-500/5 border border-rose-100 dark:border-rose-500/10 space-y-1 sm:space-y-1.5">
        <div
          v-if="!selectedDestinationName"
          class="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold text-rose-600 dark:text-rose-400"
        >
    ...
    <div class="bg-white dark:bg-[#1e293b]/60 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 rounded-2xl p-3.5 sm:p-4 shadow-sm space-y-2 sm:space-y-2.5">
      <div class="flex items-center justify-between">
        <label class="text-[10px] sm:text-[11px] font-black text-slate-400 tracking-[0.1em] uppercase opacity-70">{{ $t('warehouse.transfer.note') }}</label>
        <span class="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase opacity-60">{{ $t('warehouse.transfer.optional') }}</span>
      </div>
      <textarea
        v-model="transferForm.note"
        rows="2"
        :placeholder="$t('warehouse.transfer.note_ph')"
        class="w-full text-[13px] sm:text-[14px] font-bold rounded-xl border border-slate-200/50 dark:border-slate-800/50 bg-slate-50 dark:bg-slate-900/50 text-slate-800 dark:text-white placeholder-slate-400 p-2.5 sm:p-3 focus:outline-none focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 resize-none transition-all shadow-inner"
      ></textarea>
    </div>
  </div>
</template>
