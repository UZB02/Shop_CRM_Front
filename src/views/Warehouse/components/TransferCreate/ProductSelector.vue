    ...
    <div class="flex flex-col max-h-[85vh]">
      ...
      <div class="flex items-center justify-between px-8 py-6 border-b border-slate-100 dark:border-slate-800/50">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h3 class="text-[14px] font-black text-slate-800 dark:text-white font-outfit tracking-tight uppercase leading-none mb-1">{{ $t('warehouse.transfer.select_product') }}</h3>
            <div class="flex items-center gap-1.5 opacity-60">
              <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ sourceName }}</span>
              <i class="pi pi-chevron-right text-[7px] text-slate-300"></i>
              <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ $t('warehouse.transfer.available_products') || 'Mahsulotlar' }}</span>
            </div>
          </div>
        </div>
        <button
      ...
      <div class="px-8 py-5 border-b border-slate-100 dark:border-slate-800/50 bg-slate-50/50 dark:bg-transparent">
        <div class="relative group">
          ...
          <input
            ...
            class="w-full h-11 pl-12 pr-6 text-[13px] font-bold rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 transition-all shadow-sm"
            autofocus
          />
        </div>
      </div>
      ...
      <div class="overflow-y-auto flex-1 no-scrollbar p-4 space-y-2">
        ...
        >
          ...
          <p class="text-[12px] font-black text-slate-400 uppercase tracking-widest">{{ $t('warehouse.transfer.no_product_found') }}</p>
        </div>
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-3 mb-1">
              <span class="text-[14px] font-black text-slate-800 dark:text-white font-outfit truncate group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
            ...
            <div class="flex items-center gap-4">
              <div class="flex items-center gap-1.5">
                <i class="pi pi-barcode text-[9px] text-slate-300"></i>
                <code class="text-[10px] font-black tracking-wider text-slate-400">{{ p.barcode || 'NO BARCODE' }}</code>
              </div>
              <div class="flex items-center gap-1.5">
                <i class="pi pi-database text-[9px] text-emerald-500/60"></i>
                <span class="text-[10px] font-black text-emerald-600/80 dark:text-emerald-400/80 uppercase tracking-tighter">{{ $t('warehouse.transfer.available_qty', { qty: p.quantity }) }}</span>
              </div>
            </div>
          </div>
