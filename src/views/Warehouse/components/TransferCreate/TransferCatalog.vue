  ...
  <div class="h-full flex flex-col bg-white dark:bg-[#0f172a]">
    ...
    <div class="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800/50 space-y-3 sm:space-y-4">
      <div class="flex items-center justify-between">
        <h4 class="text-[12px] sm:text-[13px] font-black text-slate-400 tracking-[0.1em] uppercase opacity-70">{{ $t('menu.products') }}</h4>
        <div class="flex items-center gap-2">
          <span class="text-[11px] sm:text-[12px] font-bold text-slate-400 uppercase opacity-60">{{ products.length }} {{ $t('common.items') || 'items' }}</span>
        </div>
      </div>

      <!-- Search Bar -->
      <div class="relative group">
        <i class="pi pi-search absolute left-3 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-700 group-focus-within:text-emerald-500 transition-colors text-[14px]" />
        <input 
          v-model="searchQuery" 
          type="text" 
          :placeholder="$t('common.search') + '...'" 
          class="w-full h-9 sm:h-10 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-lg sm:rounded-xl pl-9 sm:pl-10 pr-4 text-[14px] sm:text-[15px] font-bold outline-none focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 placeholder:text-slate-300 transition-all shadow-inner"
        />
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto px-3 sm:px-4 py-2 sm:py-3 no-scrollbar">
      <div v-if="filteredProducts.length === 0" class="h-full flex flex-col items-center justify-center opacity-20 py-20 text-center">
         ...
         <p class="font-black text-[14px] tracking-widest uppercase">{{ $t('common.no_data') }}</p>
      </div>
          ...
          <div class="flex items-center gap-4 p-2.5 rounded-xl border border-transparent hover:border-emerald-500/30 hover:bg-emerald-50/30 dark:hover:bg-emerald-950/10 transition-all duration-200 relative overflow-hidden">
            ...
            <div class="relative shrink-0">
              ...
              <Transition name="scale">
                <div v-if="getItemQty(product)" class="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] bg-emerald-500 text-white rounded-full flex items-center justify-center text-[11px] font-black shadow-lg border-2 border-white dark:border-slate-900 z-[20]">
                  {{ getItemQty(product) }}
                </div>
              </Transition>
            </div>
            ...
            <div class="flex-1 min-w-0">
              <h5 class="text-[15px] font-black text-slate-700 dark:text-slate-300 truncate tracking-tight font-outfit">{{ product.product_name || product.name }}</h5>
              <div class="flex items-center gap-2 mt-1">
                <!-- Remaining stock badge -->
                <div class="flex items-center gap-1 opacity-70">
                  <i class="pi pi-database text-[12px]"
                    :class="remainingQty(product) === 0 ? 'text-rose-400' : remainingQty(product) <= product.quantity * 0.2 ? 'text-amber-500' : 'text-emerald-500'"
                  ></i>
                  <span class="text-[13px] font-black uppercase tracking-tighter"
                  ...
                  <span v-if="getItemQty(product) > 0" class="text-[11px] text-slate-400 font-bold">({{ $t('common.from') || 'dan' }} {{ product.quantity }})</span>
                </div>
            ...
            <div v-else class="w-8 h-8 rounded-lg bg-rose-50 dark:bg-rose-500/10 border border-rose-100 dark:border-rose-500/20 flex items-center justify-center">
              <i class="pi pi-times text-[12px] text-rose-400"></i>
            </div>
          </div>
        </div>
      </div>
