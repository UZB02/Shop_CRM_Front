  ...
  <div class="flex-1 flex flex-col min-h-0">
    ...
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-3">
        <div class="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-emerald-500/10 flex items-center justify-center border border-emerald-500/10 shrink-0">
          <i class="pi pi-shopping-cart text-emerald-500 text-[12px] sm:text-xs"></i>
        </div>
        <div>
          <h3 class="text-[14px] sm:text-[16px] font-black text-slate-800 dark:text-white font-outfit tracking-tight uppercase leading-none mb-1 opacity-70">{{ $t('warehouse.transfer.items_list') || 'Ro\'yxat' }}</h3>
          <p class="text-[11px] sm:text-[13px] font-bold text-slate-400 uppercase tracking-widest opacity-60">{{ items.length }} {{ $t('warehouse.transfer.products') }}</p>
        </div>
      </div>
      ...
      <div v-if="items.length" class="flex items-center gap-2 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200/50 dark:border-slate-700/50 shadow-inner">
        <span class="hidden xs:inline text-[12px] sm:text-[13px] font-black text-slate-500 uppercase tracking-widest opacity-60">{{ $t('warehouse.transfer.total_qty') || 'Jami' }}:</span>
        <span class="text-[13px] sm:text-[15px] font-black text-slate-900 dark:text-white">{{ totalQuantity }}</span>
      </div>
    </div>
    ...
    >
      ...
      <div class="max-w-[240px]">
        <p class="text-[16px] sm:text-[16px] font-black text-slate-800 dark:text-white font-outfit uppercase tracking-tight">{{ $t('warehouse.transfer.no_items') }}</p>
        <p class="text-[13px] sm:text-[15px] font-bold text-slate-400 dark:text-slate-500 mt-2 leading-relaxed">{{ $t('warehouse.transfer.no_items_desc') }}</p>
      </div>
    </div>
          ...
          >
            ...
            <div class="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-0.5 sm:mb-1">
                  <h4 class="text-[15px] sm:text-[16px] font-black text-slate-800 dark:text-white font-outfit truncate leading-tight uppercase">{{ item.product.product_name || item.product.name }}</h4>
                </div>
                <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
                  <div class="flex items-center gap-1 opacity-70">
                    <i class="pi pi-barcode text-[11px] text-slate-400"></i>
                    <code class="text-[12px] font-bold tracking-tight text-slate-400 truncate max-w-[80px] sm:max-w-none">{{ item.product.barcode || '—' }}</code>
                  </div>
                  <div class="flex items-center gap-1 px-1.5 py-0.5 rounded-lg bg-emerald-500/5 border border-emerald-500/10">
                    <span class="text-[11px] font-black text-emerald-500 uppercase tracking-widest opacity-70">{{ $t('warehouse.transfer.col_available') }}:</span>
                    <span class="text-[12px] sm:text-[14px] font-black text-emerald-600 dark:text-emerald-400">{{ item.product.quantity }}</span>
                  </div>
            ...
            <div class="flex items-center justify-between sm:justify-end gap-3 sm:gap-4 shrink-0 border-t sm:border-t-0 pt-2.5 sm:pt-0 border-slate-100 dark:border-slate-800/40">
              <div class="flex flex-row sm:flex-col items-center sm:items-end gap-1.5 flex-1 sm:flex-initial">
                <span class="text-[11px] sm:text-[12px] font-black text-slate-400 uppercase tracking-widest opacity-60 hidden xs:block">{{ $t('warehouse.transfer.col_quantity') }}</span>
                ...
                >
                  ...
                  <input
                    ...
                    class="w-12 sm:w-14 h-8 text-center text-[14px] sm:text-[15px] font-black bg-transparent border-none outline-none focus:ring-0 p-0 transition-colors"
                  ...
                  >
                    ...
                    <span
                      ...
                      class="absolute -top-2 -right-2 text-[9px] font-black bg-rose-500 text-white rounded px-1 leading-4 tracking-wider shadow-sm"
                    >MAX</span>
                  </button>
                </div>
              </div>
