  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 px-4 sm:px-6 py-3 sm:py-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm shrink-0">
    <div class="flex items-center gap-3 sm:gap-4">
      ...
      <div class="min-w-0">
        <div class="flex items-center gap-2 flex-wrap">
          <h1 class="text-[14px] sm:text-[16px] font-black text-slate-900 dark:text-slate-50 tracking-tight truncate">
            {{ $t('warehouse.bulk.title') }}
          </h1>
          <Tag :value="warehouseName" severity="info" class="!text-[12px] sm:!text-[14px] !font-black !rounded-md !px-2" />
        </div>
        <p class="text-[11px] sm:text-[12px] font-bold text-slate-400 tracking-[0.2em] leading-none mt-0.5">
    ...
    <div class="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100 dark:border-slate-800">
      <div class="flex flex-col items-start sm:items-end pr-4 sm:border-r border-slate-100 dark:border-slate-800">
        <span class="text-[11px] font-black text-slate-400 tracking-widest leading-none mb-1">
          {{ $t('warehouse.bulk.filled') }}
        </span>
        <span class="text-[12px] sm:text-[13px] font-black text-slate-800 dark:text-slate-200 leading-none">{{ validCount }} / {{ totalCount }}</span>
      </div>

      <div class="flex items-center gap-2">
        <button
          @click="$emit('back')"
          class="h-9 sm:h-11 px-4 sm:px-6 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-500 text-[12px] sm:text-[14px] font-black tracking-widest hover:bg-slate-50 transition-all active:scale-95"
        >
        ...
        <button
          ...
          class="h-9 sm:h-11 px-6 sm:px-8 rounded-xl text-white text-[12px] sm:text-[14px] font-black tracking-widest transition-all shadow-lg active:scale-95 disabled:opacity-30 flex items-center justify-center gap-2"
