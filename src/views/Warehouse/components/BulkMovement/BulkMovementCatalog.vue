  ...
  <div class="h-full flex flex-col bg-white dark:bg-[#0f172a]">
    ...
    <div class="p-6 border-b border-slate-100 dark:border-slate-800/50 space-y-4">
      <div class="flex items-center justify-between">
        <h4 class="text-[10px] font-black text-slate-400 tracking-[0.2em] uppercase">{{ $t('menu.products') }}</h4>
        <div v-if="loading" class="w-4 h-4 border-2 border-slate-200 border-t-emerald-500 rounded-full animate-spin"></div>
      </div>

      <!-- Search -->
      <div class="relative group">
        <i class="pi pi-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-700 group-focus-within:text-emerald-500 transition-colors text-[10px]" />
        <input 
          v-model="searchQuery" 
          type="text" 
          :placeholder="$t('common.search') + '...'" 
          class="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-xl pl-10 pr-4 py-3 text-[11px] font-bold outline-none focus:ring-2 focus:ring-emerald-500/10 placeholder:text-slate-300 transition-all"
        />
      </div>
      ...
      <div class="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
        <button 
          ...
          class="px-4 py-2 rounded-lg text-[9px] font-black tracking-widest transition-all whitespace-nowrap border"
    ...
    <div class="flex-1 overflow-y-auto px-4 py-2 custom-scrollbar">
      <div v-if="filteredProducts.length === 0 && !loading" class="h-full flex flex-col items-center justify-center opacity-20 py-10">
         ...
         <p class="font-black text-[9px] tracking-widest uppercase">{{ $t('common.no_data') }}</p>
      </div>
      ...
      <div v-else class="space-y-1.5">
        ...
        >
          <div class="flex items-center gap-3 p-2 rounded-xl border border-transparent hover:border-emerald-500/30 hover:bg-emerald-50/30 dark:hover:bg-emerald-950/10 transition-all duration-200">
            <!-- Mini Thumb -->
            <div class="w-10 h-10 rounded-lg bg-slate-50 dark:bg-slate-900 flex items-center justify-center overflow-hidden border border-slate-100 dark:border-slate-800 shrink-0 relative shadow-sm">
              ...
              <div v-if="getItemQty(product)" class="absolute -top-1 -right-1 min-w-[14px] h-[14px] bg-emerald-500 text-white rounded-full flex items-center justify-center text-[7px] font-black shadow-md border border-white dark:border-slate-800">
                {{ getItemQty(product) }}
              </div>
            </div>
            
            <div class="flex-1 min-w-0">
              <h5 class="text-[10px] font-black text-slate-700 dark:text-slate-300 truncate tracking-tight">{{ product.name }}</h5>
              <div class="flex items-center gap-2 mt-0.5">
                ...
                <span class="text-[8px] font-bold text-slate-400 tracking-tighter truncate">{{ product.barcode }}</span>
              </div>
            </div>
