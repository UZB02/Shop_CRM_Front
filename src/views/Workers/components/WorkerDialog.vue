    ...
    <div class="flex flex-col bg-white dark:bg-slate-900 rounded-xl overflow-hidden shadow-2xl" style="max-height: 90vh;">
      ...
      <div class="px-6 md:px-8 py-5 bg-slate-50/50 dark:bg-slate-800/30 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-end gap-3">
        <button
          ...
          class="w-full sm:w-auto h-11 px-6 rounded-xl text-[13px] font-black tracking-widest text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all active:scale-[0.98]"
        >
          {{ $t('common.cancel') }}
        </button>
        <button
          @click="$emit('save')"
          :disabled="saving"
          class="w-full sm:w-auto h-11 px-8 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-[13px] font-black tracking-widest transition-all shadow-xl shadow-emerald-500/20 active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none flex items-center justify-center gap-2"
        >
