  ...
  <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden flex flex-col transition-all duration-300">
    ...
    <div class="px-4 py-3 border-b border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/10 flex flex-col gap-3">
      <div class="flex items-center justify-between">
        ...
        <div class="px-2 py-0.5 bg-white dark:bg-slate-800 rounded-md border border-slate-100 dark:border-slate-700 shadow-sm flex items-center gap-1.5">
            ...
            <span class="text-[8px] font-bold text-slate-400 uppercase tracking-tighter">{{ $t('common.total') || 'Jami' }}</span>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 bg-white dark:bg-slate-900/40 p-1 rounded-xl border border-slate-100 dark:border-slate-800/50">
        <Select v-model="historyFilters.action" :options="actionOptions" showClear optionLabel="label" optionValue="value" 
                  class="!text-[9px] !font-black !uppercase !tracking-tight !h-8 !rounded-lg !bg-slate-50 dark:!bg-slate-800/40 !border-none !ring-0 flex items-center px-2"
        ...
        <DatePicker v-model="historyFilters.date_from" dateFormat="yy-mm-dd" showIcon iconDisplay="input" 
                    ...
                    inputClass="!text-[9px] !font-black !uppercase !tracking-tight !h-8 !bg-transparent !border-none !px-3"
                    placeholder="Dan" />
    ...
    <div class="overflow-x-auto">
      <div v-if="activitiesLoading && !activities.length" class="py-12 text-center">
        ...
        <p class="text-[10px] font-bold uppercase tracking-widest text-slate-400">Yuklanmoqda...</p>
      </div>
      <div v-else-if="!activities.length" class="py-12 text-center opacity-40">
        ...
        <p class="text-[10px] font-bold uppercase tracking-widest text-slate-400">Hozircha amallar mavjud emas</p>
      </div>
      <table v-else class="w-full text-left border-collapse min-w-[600px]">
        <thead>
          <tr class="bg-slate-50/50 dark:bg-slate-800/30">
            <th class="px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-slate-800">{{ $t('common.action') || 'Amal' }}</th>
            <th class="px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-slate-800">{{ $t('common.description') || 'Tavsif' }}</th>
            <th class="px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-slate-800 text-right w-32">{{ $t('common.date') || 'Sana' }}</th>
          </tr>
        </thead>
            ...
            <td class="px-4 py-2">
              <div class="flex items-center gap-2.5">
                ...
                <div class="flex flex-col">
                  <span class="text-[10px] font-bold text-slate-700 dark:text-slate-200 uppercase tracking-tight">
                    {{ act.action_label }}
                  </span>
                  <span v-if="act.model_label" class="text-[8px] font-bold text-slate-400 uppercase tracking-tighter">
                    {{ act.model_label }}
                  </span>
                </div>
              </div>
      ...
      <div v-if="hasMoreActivities" class="p-3 text-center border-t border-slate-50 dark:border-slate-800/50 bg-slate-50/10">
        <Button 
          ...
          class="!text-[9px] !font-black !uppercase !tracking-widest !text-slate-400 hover:!text-amber-500 !transition-colors !p-0 !h-auto"
        />
      </div>
    </div>
  </div>
