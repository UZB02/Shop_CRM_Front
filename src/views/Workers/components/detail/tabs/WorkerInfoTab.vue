  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2rem] overflow-hidden shadow-sm">
      <div class="px-6 py-4 border-b border-slate-50 dark:border-slate-800/50 bg-slate-50/50 dark:bg-slate-800/30 flex items-center gap-3">
        ...
        <h3 class="text-[12px] font-black tracking-widest text-slate-500 dark:text-slate-400">Shaxsiy ma'lumotlar</h3>
      </div>
      <div class="p-6 space-y-5">
        <div v-for="key in ['full_name', 'username', 'status', 'created_at']" :key="key">
          <p class="text-[11px] font-black tracking-widest text-slate-400 mb-1.5">{{ getLabel(key) }}</p>
          <div class="flex items-center gap-2">
            <span v-if="key === 'status'" class="px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-500 text-[11px] font-black ">
              {{ getValue(key) }}
            </span>
            <p v-else class="text-[14px] font-bold text-slate-800 dark:text-slate-200 tracking-tight">{{ getValue(key) }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2rem] overflow-hidden shadow-sm">
      <div class="px-6 py-4 border-b border-slate-50 dark:border-slate-800/50 bg-slate-50/50 dark:bg-slate-800/30 flex items-center gap-3">
        ...
        <h3 class="text-[12px] font-black tracking-widest text-slate-500 dark:text-slate-400">Aloqa ma'lumotlari</h3>
      </div>
      <div class="p-6 space-y-5">
        <div v-for="key in ['phone1', 'phone2', 'email']" :key="key">
          <p class="text-[11px] font-black tracking-widest text-slate-400 mb-1.5">{{ getLabel(key) }}</p>
          <p class="text-[14px] font-bold text-slate-800 dark:text-slate-200 tracking-tight flex items-center gap-2">
            <i v-if="key.includes('phone')" class="pi pi-phone text-[12px] opacity-20"></i>
            <i v-if="key === 'email'" class="pi pi-envelope text-[12px] opacity-20"></i>
            {{ getValue(key) }}
          </p>
        </div>
      </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2rem] overflow-hidden shadow-sm">
      <div class="px-6 py-4 border-b border-slate-50 dark:border-slate-800/50 bg-slate-50/50 dark:bg-slate-800/30 flex items-center gap-3">
        ...
        <h3 class="text-[12px] font-black tracking-widest text-slate-500 dark:text-slate-400">Ish faoliyati</h3>
      </div>
      <div class="p-6 space-y-5">
        <div v-for="key in ['role', 'branch']" :key="key">
          ...
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 flex items-center justify-center text-emerald-500 shadow-inner">
              <i :class="key === 'role' ? 'pi pi-id-card' : 'pi pi-map-marker'" class="text-[12px]"></i>
            </div>
            <p class="text-[14px] font-black text-slate-800 dark:text-slate-100 tracking-tight ">{{ getValue(key) }}</p>
          </div>
        </div>
      </div>
    </div>
