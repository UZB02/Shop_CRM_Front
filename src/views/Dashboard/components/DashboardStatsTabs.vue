    ...
    <div class="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:justify-between z-10 relative">
      ...
      <div class="flex items-center gap-1 overflow-x-auto no-scrollbar p-0.5 flex-1">
        <button
          ...
          :class="[
            'relative shrink-0 flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-[0.08em] sm:tracking-[0.1em] transition-all duration-300 whitespace-nowrap overflow-hidden',
            modelValue === tab.id
              ? 'text-white'
        ...
        >
          ...
          <i :class="[tab.icon, 'text-[10px] sm:text-[11px] transition-transform', modelValue === tab.id ? 'scale-110' : 'opacity-60']"></i>
          <span>{{ tab.label }}</span>
        </button>
      </div>
      ...
      <div class="flex items-center justify-between sm:justify-end gap-3 px-3 py-2 bg-slate-50 dark:bg-white/5 rounded-[0.9rem] border border-slate-100 dark:border-white/5 mx-0.5 sm:mx-0 sm:shrink-0">
        <div class="flex flex-col">
          <span class="text-[8px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Sof Foyda</span>
        ...
        <div v-if="salesCount" class="hidden md:flex flex-col items-end border-l border-slate-200 dark:border-slate-700 pl-3">
          <span class="text-[8px] font-black uppercase tracking-[0.2em] text-slate-400">Sotuvlar</span>
        ...
        <div v-if="smenaCount" class="hidden md:flex flex-col items-end border-l border-slate-200 dark:border-slate-700 pl-3">
          <span class="text-[8px] font-black uppercase tracking-[0.2em] text-emerald-500">Smena</span>
