  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
    ...
    <div v-if="type === 'sales'" class="p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
      <div class="flex items-center justify-between mb-6">
        <h3 class="text-sm font-black text-slate-800 dark:text-white tracking-tight">Ochiq Smenalar</h3>
        <div class="flex items-center gap-2">
          <span v-if="currentSmena?.open_count" class="px-2 py-0.5 rounded-lg bg-emerald-500/10 text-emerald-500 text-[9px] font-black ">{{ currentSmena.open_count }} TA</span>
          <div class="flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-emerald-500/10 text-emerald-500 text-[9px] font-black ">
      ...
      <div v-if="!currentSmena?.smenas?.length" class="flex flex-col items-center justify-center py-12 text-slate-400">
        ...
        <p class="text-[10px] font-black tracking-widest">Smenalar yo'q</p>
      </div>
      ...
      <div v-else class="space-y-4">
        <div v-for="s in currentSmena.smenas" :key="s.smena_id" class="flex items-center gap-3 group">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[10px] text-slate-400 font-bold tracking-tighter">{{ s.branch }} • {{ s.start_time }}</p>
          </div>
    ...
    <div v-if="type === 'sales'" class="p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
      <div class="flex items-center justify-between mb-6">
        <h3 class="text-sm font-black text-slate-800 dark:text-white tracking-tight">Ommabop Mahsulotlar</h3>
        <span class="px-2 py-0.5 rounded-lg bg-emerald-500/10 text-emerald-500 text-[9px] font-black ">Eng ko'p sotilgan</span>
      </div>
      <div class="space-y-4">
        <div v-for="p in products.top_selling" :key="p.product_id" class="flex items-center gap-3 group">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[10px] text-slate-400 font-bold tracking-tighter">{{ p.total_qty }} {{ p.unit }} sotilgan</p>
          </div>
          <div class="text-right">
    ...
    <div v-if="type === 'sales'" class="p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
      <div class="flex items-center justify-between mb-6">
        <h3 class="text-sm font-black text-slate-800 dark:text-white tracking-tight">Eng Foydali</h3>
        <span class="px-2 py-0.5 rounded-lg bg-blue-500/10 text-blue-500 text-[9px] font-black ">Foyda bo'yicha</span>
      </div>
      <div class="space-y-4">
        <div v-for="p in products.top_profitable" :key="p.product_id" class="flex items-center gap-3 group">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-[11px] font-black text-slate-800 dark:text-white truncate">{{ p.name }}</p>
            <p class="text-[10px] text-slate-400 font-bold tracking-tighter">Tushum: {{ formatPrice(p.total_revenue) }}</p>
          </div>
          <div class="text-right">
    ...
    <div v-if="type === 'sales'" class="md:col-span-2 lg:col-span-3 p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
      <div class="flex items-center justify-between mb-6">
        <h3 class="text-sm font-black text-slate-800 dark:text-white tracking-tight">Sotuvchilar Reytingi</h3>
        <span v-if="workers.total_active_workers" class="text-[9px] font-black text-slate-400 tracking-widest">{{ workers.total_active_workers }} ta faol</span>
      </div>
      <div class="overflow-x-auto no-scrollbar">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="text-[9px] font-black text-slate-400 tracking-widest border-b border-slate-50 dark:border-slate-800">
          ...
          <tbody class="divide-y divide-slate-50 dark:divide-slate-800/50">
            <tr v-for="(w, idx) in workers.ranking" :key="w.worker_id" class="group hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
              ...
              <td class="py-4">
                <span :class="['px-2 py-0.5 rounded-md text-[8px] font-black ', 
    ...
    <div v-if="type === 'inventory'" class="md:col-span-2 lg:col-span-3 p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
      <div class="flex items-center justify-between mb-6">
        <h3 class="text-sm font-black text-slate-800 dark:text-white tracking-tight">Filiallar Samaradorligi</h3>
        ...
        <div v-for="b in branches" :key="b.branch_id" class="p-4 rounded-2xl bg-slate-50/50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-4">
           <div class="flex items-center gap-3">
              ...
              <div>
                 ...
                 <p class="text-[9px] font-black text-slate-400 tracking-tighter">{{ b.count }} sotuv • Kapital: {{ formatPrice(b.warehouse_value) }}</p>
              </div>
           </div>
           <div class="flex gap-8 ml-auto">
              <div class="text-right">
                 <p class="text-[8px] font-black text-slate-400 tracking-widest">Tushum</p>
                 <p class="text-xs font-black text-slate-800 dark:text-white">{{ formatPrice(b.revenue) }}</p>
              </div>
              <div class="text-right">
                 <p class="text-[8px] font-black text-slate-400 tracking-widest">Foyda</p>
    ...
    <div v-if="type === 'finance'" class="md:col-span-2 lg:col-span-3 p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
      <div class="flex items-center justify-between mb-6">
        <h3 class="text-sm font-black text-slate-800 dark:text-white tracking-tight">Yetkazib Beruvchilar</h3>
        <p class="text-[10px] font-black text-rose-500 tracking-[0.2em] px-3 py-1 rounded-full bg-rose-500/5 border border-rose-500/10">Jami Qarz: {{ formatPrice(suppliers.total_debt) }}</p>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div v-for="s in suppliers.top_debtors" :key="s.supplier_id" class="p-4 rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 flex items-center gap-3 group hover:border-rose-500/30 transition-all">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[9px] text-slate-400 font-bold tracking-tighter">{{ s.company }}</p>
          </div>
          <div class="text-right">
    ...
    <div v-if="type === 'customers'" class="md:col-span-2 lg:col-span-3 p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
      <div class="flex items-center justify-between mb-6">
        <h3 class="text-sm font-black text-slate-800 dark:text-white tracking-tight">VIP Mijozlar</h3>
        <span class="px-2 py-0.5 rounded-lg bg-blue-500/10 text-blue-500 text-[9px] font-black ">Xaridlar bo'yicha</span>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="c in customers.top_buyers" :key="c.customer_id" class="flex items-center gap-3 group p-4 rounded-2xl bg-slate-50/50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
           ...
           <div class="flex-1 min-w-0">
             ...
             <p class="text-[10px] text-slate-400 font-bold tracking-tighter">{{ c.visit_count }} ta xarid</p>
           </div>
           <div class="text-right">
