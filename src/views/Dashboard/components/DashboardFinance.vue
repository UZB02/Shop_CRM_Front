  ...
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
      ...
      <div class="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col gap-3">
        ...
        <div class="flex items-center justify-between text-[9px] font-black uppercase tracking-wider">
      ...
      <div class="p-5 rounded-3xl bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 border border-emerald-500/20 shadow-sm flex flex-col gap-3">
        ...
        <div class="text-[9px] font-black text-emerald-600/70 uppercase tracking-wider">
    ...
    <div class="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
      ...
      <div class="flex flex-wrap items-center justify-center gap-3 text-center">
        ...
        <div class="flex flex-col gap-1">
          <p class="text-[8px] font-black text-orange-400 uppercase tracking-widest">COGS</p>
          <p class="text-base font-black text-orange-500">{{ formatPrice(expenses.cogs) }}</p>
        </div>
        <span class="text-xl font-black text-slate-300">−</span>
        <div class="flex flex-col gap-1">
          <p class="text-[8px] font-black text-rose-400 uppercase tracking-widest">Xarajat</p>
          <p class="text-base font-black text-rose-500">{{ formatPrice(expenses.total) }}</p>
        </div>
        <span class="text-xl font-black text-slate-300">−</span>
        <div class="flex flex-col gap-1">
          <p class="text-[8px] font-black text-orange-400 uppercase tracking-widest">Isrof</p>
        ...
        <div class="flex flex-col gap-1 px-4 py-2 rounded-2xl bg-emerald-500/10 border border-emerald-500/20">
          <p class="text-[8px] font-black text-emerald-500 uppercase tracking-widest">Sof Foyda</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div class="lg:col-span-2 p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
        <div class="flex items-center justify-between mb-6">
          <div>
            <h3 class="text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight">Xarajat Kategoriyalari</h3>
            <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Davr bo'yicha taqsimot</p>
          </div>
        ...
        <div class="space-y-4">
          ...
          <div v-if="!expenses.by_category?.length" class="text-center py-8 text-slate-400">
            ...
            <p class="text-[9px] font-black uppercase tracking-widest mt-2">Ma'lumot yo'q</p>
          </div>
        </div>
      </div>
      ...
      <div class="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center">
        <h3 class="text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight mb-6 self-start">Nisbat</h3>
        ...
        <div class="mt-4 space-y-2 w-full">
          <div v-for="(cat, i) in expenses.by_category" :key="cat.category_id" class="flex items-center gap-2">
            ...
            <span class="text-[9px] font-black text-slate-500 flex-1 uppercase">{{ cat.name }}</span>
