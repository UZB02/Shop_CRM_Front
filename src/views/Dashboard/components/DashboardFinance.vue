<template>
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
      ...
      <div class="p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col gap-2 sm:gap-3">
        <div class="flex items-center justify-between">
          <span class="text-[11px] font-black text-slate-400 tracking-[0.2em]">{{ $t('dashboard.finance.gross_revenue') }}</span>
        ...
        <div class="flex items-center justify-between text-[11px] font-black tracking-wider">
          <span class="text-slate-400">{{ $t('dashboard.finance.discount') }}</span>
      ...
      <div class="p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col gap-2 sm:gap-3">
        <div class="flex items-center justify-between">
          <span class="text-[11px] font-black text-slate-400 tracking-[0.2em]">{{ $t('dashboard.finance.cogs') }}</span>
        ...
        <div class="flex items-center justify-between text-[11px] font-black tracking-wider">
          <span class="text-slate-400">{{ $t('dashboard.metrics.wastage') }}</span>
      ...
      <div class="p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col gap-2 sm:gap-3">
        <div class="flex items-center justify-between">
          <span class="text-[11px] font-black text-slate-400 tracking-[0.2em]">{{ $t('dashboard.metrics.expense') }}</span>
        ...
        <div class="flex items-center justify-between text-[11px] font-black tracking-wider">
          <span class="text-slate-400">{{ $t('dashboard.finance.ratio') }}</span>
          <span class="text-rose-500">{{ expenses.expense_ratio }}% {{ $t('dashboard.charts.revenue').toLowerCase() }}</span>
        </div>
      </div>
      ...
      <div class="p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 border border-emerald-500/20 shadow-sm flex flex-col gap-2 sm:gap-3">
        <div class="flex items-center justify-between">
          <span class="text-[11px] font-black text-emerald-600 dark:text-emerald-400 tracking-[0.2em]">{{ $t('dashboard.metrics.net_profit') }}</span>
        ...
        <div class="text-[11px] font-black text-emerald-600/70 tracking-wider">
          {{ $t('dashboard.charts.revenue') }}: {{ formatPrice(sales.total_revenue) }}
        </div>
      </div>
    </div>
    ...
    <div class="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
      <h3 class="text-sm font-black text-slate-800 dark:text-white tracking-tight mb-6">{{ $t('dashboard.finance.profit_calc') }}</h3>
      <div class="flex flex-wrap items-center justify-center gap-2 sm:gap-3 text-center">
        ...
        <span class="text-xl font-black text-slate-300">−</span>
        <div class="flex flex-col gap-1">
          <p class="text-[10px] font-black text-rose-400 tracking-widest">{{ $t('dashboard.metrics.expense') }}</p>
          <p class="text-base font-black text-rose-500">{{ formatPrice(expenses.total) }}</p>
        </div>
        <span class="text-xl font-black text-slate-300">−</span>
        <div class="flex flex-col gap-1">
          <p class="text-[10px] font-black text-orange-400 tracking-widest">{{ $t('dashboard.metrics.wastage') }}</p>
        ...
        <div class="flex flex-col gap-1 px-4 py-2 rounded-2xl bg-emerald-500/10 border border-emerald-500/20">
          <p class="text-[10px] font-black text-emerald-500 tracking-widest">{{ $t('dashboard.metrics.net_profit') }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div class="lg:col-span-2 p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
        <div class="flex items-center justify-between mb-6">
          <div>
            <h3 class="text-sm font-black text-slate-800 dark:text-white tracking-tight">{{ $t('dashboard.finance.categories') }}</h3>
            <p class="text-[11px] text-slate-400 font-bold tracking-widest mt-0.5">{{ $t('dashboard.finance.period_dist') }}</p>
          </div>
        ...
        <div class="space-y-4">
          ...
          <div v-if="!expenses.by_category?.length" class="text-center py-8 text-slate-400">
            ...
            <p class="text-[11px] font-black tracking-widest mt-2">{{ $t('dashboard.finance.no_data') }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center">
        <h3 class="text-sm font-black text-slate-800 dark:text-white tracking-tight mb-6 self-start">{{ $t('dashboard.finance.ratio') }}</h3>
    ...
    <div v-if="expenses.trend_3months?.length" class="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
      <h3 class="text-sm font-black text-slate-800 dark:text-white tracking-tight mb-6">{{ $t('dashboard.finance.expense_trend') }}</h3>
      ...
      <div v-else class="h-[200px] flex items-center justify-center border border-dashed border-slate-100 dark:border-slate-800 rounded-2xl">
        <p class="text-[11px] font-black text-slate-400 tracking-widest">{{ $t('dashboard.finance.trend_loading') }}</p>
      </div>
    </div>
  </div>
</template>
...
const trendChartData = computed(() => ({
  ...
  datasets: [
    ...
    {
      label: t('dashboard.metrics.expense'),
