  ...
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
      <div v-for="stat in mainStats" :key="stat.label" class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm flex items-center gap-4">
        ...
        <div class="min-w-0">
          <p class="text-[12px] font-bold tracking-widest text-slate-400">{{ stat.label }}</p>
    ...
    <div v-if="isManager && summaryData.totalExpenses > 0" class="bg-rose-500/5 border border-rose-500/10 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-[12px] font-bold text-slate-500 dark:text-slate-400">{{ $t('reports.financial_desc') }}</p>
        </div>
      </div>
      <div class="flex items-center gap-6">
        <div class="text-center sm:text-right">
          <span class="text-[11px] font-black tracking-widest text-slate-400 block mb-0.5">{{ $t('reports.total_ops') }}</span>
        ...
        <button 
          ...
          class="h-9 px-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-[12px] font-black tracking-widest text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm"
        >
          {{ $t('common.view_all') }}
        </button>
      </div>
