    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
       ...
       <div class="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] overflow-hidden shadow-sm flex flex-col">
          <div class="px-8 py-6 border-b border-slate-50 dark:border-slate-800 bg-emerald-500/5 flex items-center justify-between">
             <div class="flex items-center gap-3">
                ...
                <h3 class="text-[11px] font-black tracking-[0.2em] text-emerald-600">Top Sotuvchilar</h3>
             </div>
             <span class="text-[10px] font-black px-3 py-1.5 rounded-lg bg-white dark:bg-slate-800 text-emerald-500 border border-emerald-100 dark:border-emerald-500/10 tracking-widest">DAROMAD BO'YICHA</span>
          </div>
          
          <div class="overflow-x-auto">
             <table class="w-full text-left border-collapse">
                <thead>
                   <tr class="text-[10px] font-black tracking-widest text-slate-400 border-b border-slate-50 dark:border-slate-800/50">
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800/40">
                   <tr v-for="(item, index) in data" :key="item.worker_id" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/10 transition-all duration-300 group">
                      ...
                      <td class="px-8 py-5">
                         <div class="flex items-center gap-3">
                            ...
                            <div>
                               ...
                               <p class="text-[10px] font-black text-slate-400 tracking-widest mt-0.5">{{ item.role }}</p>
                            </div>
                         </div>
                      </td>
                      ...
                      <td class="px-8 py-5 text-right">
                         ...
                         <p class="text-[9px] font-black text-rose-500 tracking-widest mt-1" v-if="item.returns_count > 0">
                   ...
                   <tr v-if="!data.length">
                      <td colspan="5" class="px-8 py-20 text-center">
                         ...
                         <p class="text-[11px] font-black text-slate-400 tracking-[0.3em]">Ma'lumotlar mavjud emas</p>
                      </td>
                   </tr>
                </tbody>
             </table>
       ...
       <div class="flex flex-col gap-8">
          <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-8 shadow-sm flex flex-col h-full">
             <h3 class="text-[11px] font-black tracking-[0.2em] text-slate-500 mb-8 border-b border-slate-50 dark:border-slate-800 pb-4">O'rtacha Savdo (Chek)</h3>
             <div class="space-y-6 flex-1">
                <div v-for="item in data.slice(0, 5)" :key="'avg-' + item.worker_id" class="flex flex-col gap-2 group/item">
                   <div class="flex justify-between items-center">
                      <span class="text-[10px] font-black text-slate-500 tracking-widest truncate w-40 group-hover/item:text-emerald-500 transition-colors">{{ item.name }}</span>
          ...
          <div class="p-8 rounded-[2.5rem] bg-gradient-to-br from-emerald-600 to-emerald-700 text-white shadow-xl shadow-emerald-500/20 relative overflow-hidden flex flex-col justify-end min-h-[180px]">
             ...
             <div class="relative z-10">
                <p class="text-[10px] font-black tracking-[0.3em] opacity-70 mb-2">Xodimlar faolligi</p>
                ...
                <div v-if="data[0]" class="flex items-center gap-4 bg-white/10 backdrop-blur-xl p-4 rounded-2xl border border-white/20 shadow-xl transition-all duration-500 hover:bg-white/20">
                   ...
                   <div>
                      ...
                      <p class="text-[10px] font-black tracking-widest opacity-80 mt-0.5">{{ data[0].sales_count }} ta savdo</p>
                   </div>
                </div>
             </div>
          </div>
