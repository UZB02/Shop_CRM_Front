    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
       ...
       <div class="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] overflow-hidden shadow-sm flex flex-col">
          <div class="px-8 py-6 border-b border-slate-50 dark:border-slate-800 bg-emerald-500/5 flex items-center justify-between">
             <div class="flex items-center gap-3">
                ...
                <h3 class="text-[11px] font-black uppercase tracking-[0.2em] text-emerald-600">Top Sotuvchilar</h3>
             </div>
             <span class="text-[10px] font-black px-3 py-1.5 rounded-lg bg-white dark:bg-slate-800 text-emerald-500 border border-emerald-100 dark:border-emerald-500/10 uppercase tracking-widest">DAROMAD BO'YICHA</span>
          </div>
             ...
             <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800/40">
                   <tr v-for="(item, index) in data" :key="item.worker_id" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/10 transition-all duration-300 group">
                      <td class="px-8 py-5">
                         <span class="text-[10px] font-black w-6 h-6 rounded-full flex items-center justify-center transition-all duration-500 group-hover:scale-110 shadow-sm" 
                      ...
                      <td class="px-8 py-5">
                         <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[11px] font-black text-slate-400 border border-slate-200 dark:border-slate-700 shadow-inner group-hover:border-emerald-500/50 transition-colors">
                               {{ item.name.charAt(0) }}
                            </div>
                            <div>
                               <p class="text-sm font-black text-slate-800 dark:text-slate-100 group-hover:text-emerald-500 transition-colors">{{ item.name }}</p>
                               <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{{ item.role }}</p>
                            </div>
                         </div>
                      </td>
                      <td class="px-8 py-5 hidden sm:table-cell">
                         <span class="text-xs font-bold text-slate-500 uppercase tracking-tight">{{ item.branch || '—' }}</span>
                      </td>
                      <td class="px-8 py-5 text-center">
                         <span class="px-3 py-1 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 text-[10px] font-black text-emerald-600 border border-emerald-100 dark:border-emerald-500/10">
                            {{ item.sales_count }}
                         </span>
                      </td>
                      <td class="px-8 py-5 text-right">
                         <p class="text-sm font-black text-slate-800 dark:text-slate-100 tracking-tight">{{ formatCurrency(item.revenue) }}</p>
                         <p class="text-[9px] font-black text-rose-500 uppercase tracking-widest mt-1" v-if="item.returns_count > 0">
                   ...
                   <tr v-if="!data.length">
                      <td colspan="5" class="px-8 py-20 text-center">
                         ...
                         <p class="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em]">Ma'lumotlar mavjud emas</p>
                      </td>
                   </tr>
                </tbody>
             </table>
       ...
       <div class="flex flex-col gap-8">
          <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-8 shadow-sm flex flex-col h-full">
             <h3 class="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 mb-8 border-b border-slate-50 dark:border-slate-800 pb-4">O'rtacha Savdo (Chek)</h3>
             <div class="space-y-6 flex-1">
                <div v-for="item in data.slice(0, 5)" :key="'avg-' + item.worker_id" class="flex flex-col gap-2 group/item">
                   <div class="flex justify-between items-center">
                      <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest truncate w-40 group-hover/item:text-emerald-500 transition-colors">{{ item.name }}</span>
          ...
          <div class="p-8 rounded-[2.5rem] bg-gradient-to-br from-emerald-600 to-emerald-700 text-white shadow-xl shadow-emerald-500/20 relative overflow-hidden flex flex-col justify-end min-h-[180px]">
             ...
             <div class="relative z-10">
                <p class="text-[10px] font-black uppercase tracking-[0.3em] opacity-70 mb-2">Xodimlar faolligi</p>
                ...
                <div v-if="data[0]" class="flex items-center gap-4 bg-white/10 backdrop-blur-xl p-4 rounded-2xl border border-white/20 shadow-xl transition-all duration-500 hover:bg-white/20">
                   ...
                   <div>
                      ...
                      <p class="text-[10px] font-black uppercase tracking-widest opacity-80 mt-0.5">{{ data[0].sales_count }} ta savdo</p>
                   </div>
                </div>
             </div>
          </div>
