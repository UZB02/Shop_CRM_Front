    ...
    <div v-if="activeSubTab === 'analytics'" class="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-400">
      ...
      <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-4 shadow-sm">
         <div class="flex items-center justify-between mb-4">
            <div>
              <h3 class="text-[11px] font-bold tracking-widest text-slate-400 mb-0.5">{{ t('reports.expenses_flow') }}</h3>
              <p class="text-sm font-bold text-slate-800 dark:text-slate-100 tracking-tight">{{ t('reports.expenses_analysis_time') }}</p>
            </div>
    ...
    <div v-if="activeSubTab === 'purchases'" class="animate-in fade-in slide-in-from-bottom-2 duration-400">
       <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden h-fit flex flex-col">
             ...
             <table class="w-full text-left border-collapse">
                <thead>
                   <tr class="text-[11px] font-bold tracking-widest text-slate-400 border-b border-slate-50 dark:border-slate-800/50 sticky top-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md z-20">
                      ...
                      <th class="px-4 py-3">{{ t('reports.supplier_branch') }}</th>
                      <th class="px-4 py-3 text-center">{{ t('common.quantity') }} / {{ t('common.price') }}</th>
          ...
          <div class="px-4 py-2.5 border-t border-slate-50 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/10 flex justify-between items-center">
             <span class="text-[11px] font-bold tracking-widest text-slate-400">{{ t('common.total') }} {{ t('finance.purchases').toLowerCase() }}:</span>
