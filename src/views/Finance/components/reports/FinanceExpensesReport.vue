  ...
  <div class="space-y-4">
      ...
      <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-4 shadow-sm">
         <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-1.5">
               ...
               <span class="text-[10px] font-medium text-slate-400">{{ t('finance.title') }}</span>
            </div>
         </div>
         <div class="h-[180px]">
    ...
    <div v-if="activeSubTab === 'purchases'" class="animate-in fade-in slide-in-from-bottom-2 duration-400">
       <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden h-fit flex flex-col">
             ...
             <table class="w-full text-left border-collapse">
                <thead>
                   <tr class="text-[9px] font-bold uppercase tracking-widest text-slate-400 border-b border-slate-50 dark:border-slate-800/50 sticky top-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md z-20">
                      ...
                      <th class="px-4 py-3 text-right">{{ t('finance.amount') }}</th>
                   </tr>
                </thead>
                   ...
                   <tr v-for="(p, i) in props.data.purchases?.items" :key="i" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/10 transition-colors group">
                      ...
                      <td class="px-4 py-3">
                         <div class="flex flex-col">
                            ...
                            <span class="text-[9px] font-medium text-slate-400 mt-0.5">{{ p.supplier }}</span>
                         </div>
                      </td>
          ...
          <div class="px-4 py-2.5 border-t border-slate-50 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/10 flex justify-between items-center mt-auto">
             <span class="text-[9px] font-bold uppercase tracking-widest text-slate-400">{{ t('finance.total_revenue') }}:</span>
    ...
    <div v-if="activeSubTab === 'expenses'" class="animate-in fade-in slide-in-from-bottom-2 duration-400">
       <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden h-fit flex flex-col">
          <div class="px-4 py-3 border-b border-slate-50 dark:border-slate-800 bg-rose-500/5 flex justify-between items-center">
                 <div class="flex items-center gap-2">
                  <div class="w-6 h-6 rounded-lg bg-rose-500 text-white flex items-center justify-center shadow-sm">
                    <i class="pi pi-receipt text-[10px]"></i>
                  </div>
                  <span class="text-[10px] font-bold uppercase tracking-widest text-rose-600">Amaliy xarajatlar</span>
               </div>
               <span class="text-[9px] font-bold text-rose-500 uppercase tracking-widest bg-white dark:bg-slate-800 px-2 py-1 rounded-md border border-rose-100 dark:border-rose-500/10">{{ props.data.expenses?.count || 0 }} ta</span>
          </div>
          <div class="overflow-x-auto max-h-[400px] custom-scrollbar overflow-y-auto">
             <table class="w-full text-left border-collapse">
                <thead>
                   <tr class="text-[9px] font-bold uppercase tracking-widest text-slate-400 border-b border-slate-50 dark:border-slate-800/50 sticky top-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md z-20">
                      <th class="px-4 py-3">{{ t('finance.date') }}</th>
                      <th class="px-4 py-3">Kategoriya / Izoh</th>
                      <th class="px-4 py-3 text-right">{{ t('finance.amount') }}</th>
                   </tr>
                </thead>
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800/40">
                   <tr v-for="(e, i) in props.data.expenses?.items" :key="i" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/10 transition-colors group">
                      <td class="px-4 py-3 text-[10px] font-medium text-slate-400">{{ e.date }}</td>
                      <td class="px-4 py-3">
                         <div class="flex flex-col">
                            <span class="text-xs font-bold text-slate-700 dark:text-slate-200 group-hover:text-rose-500 transition-colors">{{ e.category }}</span>
                            <span class="text-[9px] font-medium text-slate-400 italic mt-0.5">{{ e.description }}</span>
                         </div>
                      </td>
                      <td class="px-4 py-3 text-xs font-bold text-rose-500 text-right">{{ formatPrice(e.amount) }}</td>
                   </tr>
                </tbody>
             </table>
          </div>
          <div class="px-4 py-2.5 border-t border-slate-50 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/10 flex justify-between items-center mt-auto">
             <span class="text-[9px] font-bold uppercase tracking-widest text-slate-400">Jami:</span>
             <span class="text-xs font-bold text-slate-800 dark:text-slate-100">{{ formatPrice(props.data.expenses?.total || 0) }}</span>
          </div>
       </div>
    </div>
  </div>
...
const subTabs = computed(() => [
  ...
  { id: 'expenses', label: 'finance.title', icon: 'pi pi-wallet' }
])
