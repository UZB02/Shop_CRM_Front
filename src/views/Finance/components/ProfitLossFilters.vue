    ...
    <div class="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-3xl p-4 shadow-xl shadow-slate-200/50 dark:shadow-none flex flex-wrap items-end gap-5">
      ...
      <div class="flex flex-col gap-2 min-w-[120px]">
         <div class="flex items-center gap-1.5 ml-1">
            ...
            <span class="text-[10px] font-black tracking-widest text-slate-500 dark:text-slate-400">Yil</span>
         </div>
         <DatePicker
           v-model="yearDate"
           view="year"
      ...
      <div class="flex flex-col gap-2 flex-grow min-w-[300px]">
         <div class="flex items-center justify-between ml-1">
            <div class="flex items-center gap-1.5">
               ...
               <span class="text-[10px] font-black tracking-widest text-slate-500 dark:text-slate-400">Hisobot oylari</span>
            </div>
            ...
            <div class="flex items-center gap-3 bg-slate-100/50 dark:bg-slate-800/50 px-3 py-1 rounded-full border border-slate-200/50 dark:border-slate-700/50">
               <button @click="setMonths('all')" class="text-[8px] font-black tracking-tight text-rose-500 hover:scale-105 transition-all">Barchasi</button>
               <div class="w-px h-2 bg-slate-200 dark:bg-slate-700"></div>
               <div class="flex gap-2.5">
                  <button @click="setMonths('q1')" :class="isQuarterActive('q1') ? 'text-rose-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'" class="text-[8px] font-black tracking-tight transition-all">1-Chorak</button>
                  <button @click="setMonths('q2')" :class="isQuarterActive('q2') ? 'text-rose-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'" class="text-[8px] font-black tracking-tight transition-all">2-Chorak</button>
                  <button @click="setMonths('q3')" :class="isQuarterActive('q3') ? 'text-rose-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'" class="text-[8px] font-black tracking-tight transition-all">3-Chorak</button>
                  <button @click="setMonths('q4')" :class="isQuarterActive('q4') ? 'text-rose-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'" class="text-[8px] font-black tracking-tight transition-all">4-Chorak</button>
               </div>
            </div>
         </div>
         ...
         <MultiSelect
           ...
           pt:chip:root:class="!bg-rose-50 dark:!bg-rose-500/10 !text-rose-600 dark:!text-rose-400 !text-[10px] !font-black !px-3 !py-1 !rounded-xl !border-none"
      ...
      <div v-if="isManager" class="flex flex-col gap-2 min-w-[200px]">
         <div class="flex items-center gap-1.5 ml-1">
            ...
            <span class="text-[10px] font-black tracking-widest text-slate-500 dark:text-slate-400">Filial</span>
         </div>
         <Select
      ...
      <div class="shrink-0">
        <button 
          @click="$emit('refresh')"
          class="h-11 px-8 rounded-2xl bg-rose-500 hover:bg-rose-600 active:scale-95 text-white text-[11px] font-black tracking-widest transition-all flex items-center gap-3 shadow-lg shadow-rose-500/30 group"
        >
