          ...
          <tbody class="divide-y divide-slate-50 dark:divide-slate-800/50">
            <tr v-for="worker in workers" :key="worker.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
              ...
              <td class="px-6 py-4 text-right">
                <span 
                  class="px-2 py-0.5 rounded-full text-[11px] font-black border uppercase tracking-widest"
        ...
        >
          ...
          <div class="flex items-start justify-between gap-3">
            <div class="flex items-center gap-3 min-w-0">
              <div class="w-12 h-12 rounded-xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 font-extrabold group-hover:bg-emerald-500 group-hover:text-white transition-all shadow-sm shrink-0">
                {{ worker.full_name?.charAt(0) }}
              </div>
              <div class="min-w-0">
                <p class="text-[14px] font-black text-slate-800 dark:text-slate-100 tracking-tight truncate">{{ worker.full_name }}</p>
                <div class="flex items-center gap-1.5 mt-0.5">
                  <span class="text-[10px] font-black text-slate-500 dark:text-slate-400 tracking-widest uppercase px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-700">{{ worker.role_display }}</span>
                </div>
              </div>
            </div>
            
            <span 
              class="px-1.5 py-0.5 rounded text-[9px] font-black border uppercase tracking-widest shrink-0"
              :class="worker.status === 'active' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-rose-500/10 text-rose-500 border-rose-500/20'"
            >
              {{ worker.status }}
            </span>
          ...
          <!-- Bottom Row: Phone & Salary -->
          <div class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between">
            <div class="flex items-center gap-1.5 text-[12px] font-bold text-slate-500 dark:text-slate-400">
              <i class="pi pi-phone text-[10px] text-emerald-500/60"></i>
              {{ worker.phone || '—' }}
            </div>
            <div class="flex items-baseline gap-1">
              <span class="text-[14px] font-black text-slate-800 dark:text-slate-200">{{ Number(worker.salary).toLocaleString() }}</span>
              <span class="text-[9px] font-black text-slate-400 uppercase">UZS</span>
            </div>
          </div>
        </router-link>
      </div>
