<template>
  <div class="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden min-h-[300px]">
    <div v-if="!products?.length" class="flex flex-col items-center justify-center py-20 opacity-40">
      <i class="pi pi-box text-4xl mb-4 text-slate-300"></i>
      <p class="text-[10px] font-black uppercase tracking-widest">{{ $t('stores.branch_detail.no_products') }}</p>
    </div>
    <div v-else class="overflow-x-auto">
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-slate-50 dark:divide-slate-800/50">
          <tr v-for="product in products" :key="product.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
            <td class="px-6 py-4">
