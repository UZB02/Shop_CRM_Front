  ...
  <div class="flex-1 flex flex-col gap-4 overflow-hidden">
    ...
    <div class="flex items-center gap-2 overflow-x-auto pb-1 custom-scrollbar no-scrollbar flex-shrink-0">
      <button 
        ...
        class="px-6 py-2.5 rounded-xl text-[12px] font-black tracking-widest transition-all whitespace-nowrap shadow-sm border"
    ...
    <div v-if="activeSubcategories.length > 0" class="flex items-center gap-2 overflow-x-auto pb-2 custom-scrollbar no-scrollbar flex-shrink-0">
      <button 
        ...
        class="px-4 py-1.5 rounded-lg text-[11px] font-bold tracking-tight transition-all whitespace-nowrap border"
        ...
        <div class="relative bg-white dark:bg-[#0f172a] rounded-2xl overflow-hidden flex flex-col border transition-all duration-300 shadow-[0_4px_12px_-4px_rgba(0,0,0,0.03)] dark:shadow-none hover:-translate-y-1 hover:shadow-lg cursor-pointer min-h-[220px]"
          ...
          <div class="h-36 w-full bg-white flex items-center justify-center relative overflow-hidden group-hover:opacity-95 transition-opacity p-3">
            ...
            <div v-if="product.active_promotion" class="absolute top-2 left-2 bg-rose-500 text-white flex flex-row px-2 py-1.5 rounded-[8px] items-center gap-1.5 max-w-[90%] text-left shadow-lg z-20 overflow-hidden">
              <span class="text-[13px] font-black leading-none shrink-0">-{{ Math.round(product.active_promotion.discount_pct || (product.sale_price ? ((product.sale_price - product.active_promotion.discounted_price) / product.sale_price * 100) : 0)) }}%</span>
              <span class="text-[10px] font-black tracking-tight leading-none truncate opacity-90">{{ product.active_promotion.name }}</span>
            </div>
          </div>
              ...
              <div class="flex justify-between items-start mb-3">
                <div class="flex flex-col gap-1">
                  ...
                  <div v-if="product.has_tur || product.tur_name" class="flex items-center gap-1">
                    <span v-if="product.tur_name" class="px-1.5 py-0.5 rounded-md text-[10px] font-black bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 tracking-widest leading-none">
                      {{ product.tur_name }}
                    </span>
                    <span v-if="product.tur_color" class="px-1.5 py-0.5 rounded-md text-[10px] font-black bg-slate-100 dark:bg-slate-800 text-slate-500 border border-slate-200 dark:border-slate-700 tracking-widest leading-none">
                      {{ product.tur_color }}
                    </span>
                  </div>
                </div>
              ...
              <div class="flex items-end justify-between mt-auto">
                <div class="flex flex-col justify-end leading-none gap-1">
                  ...
                  <div v-if="product.active_promotion" class="flex flex-col gap-0.5">
                    <span class="text-[12px] font-bold text-slate-400 line-through">{{ settingsStore.formatPrice(product.sale_price, product.currency_code) }}</span>
                  ...
                  <div v-else class="flex flex-col gap-0.5">
                    <span class="text-[12px] font-bold text-transparent select-none">&nbsp;</span>
