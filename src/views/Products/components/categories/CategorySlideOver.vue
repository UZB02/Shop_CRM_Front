          ...
          <div class="space-y-8">
            ...
            <div class="flex items-center justify-between p-5 bg-slate-50 dark:bg-slate-900/60 rounded-[1.5rem] border border-slate-100 dark:border-slate-800/80 group/status transition-all hover:bg-white dark:hover:bg-slate-900 hover:shadow-xl hover:shadow-slate-200/50 dark:hover:shadow-none">
              <div class="flex items-center gap-4">
                <div :class="['w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-500', category.status === 'active' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30' : 'bg-slate-200 dark:bg-slate-800 text-slate-400']">
                  <i :class="['pi', category.status === 'active' ? 'pi-check' : 'pi-power-off', 'text-[10px] font-black']"></i>
                </div>
                <div>
                  <p class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 mb-0.5">{{ $t('common.status') }}</p>
                  <p :class="['text-[11px] font-black uppercase tracking-widest transition-colors duration-300', category.status === 'active' ? 'text-emerald-500' : 'text-slate-400']">
