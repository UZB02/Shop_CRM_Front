  ...
  <div class="pt-[2rem] sm:pt-[2.5rem] pb-4 sm:pb-6 px-4 sm:px-6 rounded-b-[2rem] sm:rounded-b-[2.5rem] border-x border-b border-slate-100 dark:border-slate-800/60 bg-slate-50/30 dark:bg-slate-800/10 backdrop-blur-sm">
    ...
    <div class="grid grid-cols-1 gap-2 sm:gap-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar overscroll-contain">
      <div 
        ...
        :key="sub.id || sub._id"
        class="group/sub relative p-3 sm:p-4 rounded-xl sm:rounded-2xl transition-all duration-300 border"
        :class="[
          sub.status === 'inactive' 
            ? 'bg-rose-50/50 dark:bg-rose-950/20 border-rose-200 dark:border-rose-800/50 hover:border-rose-400 dark:hover:border-rose-700' 
            : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 hover:border-emerald-500/30 hover:shadow-xl hover:shadow-emerald-500/5'
        ]"
      >
        <div class="flex items-center justify-between transition-all gap-4">
          <div class="overflow-hidden flex-1 flex items-center gap-4">
            <div 
              ...
              :class="[
                sub.status === 'inactive' 
                  ? 'bg-rose-500' 
                  : 'bg-emerald-500/20 group-hover/sub:bg-emerald-500'
              ]"
            ></div>
          ...
          <div class="flex items-center gap-1 sm:gap-1.5 shrink-0 transition-all">
