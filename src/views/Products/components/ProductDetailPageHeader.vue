  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <div class="min-w-0">
      <div class="flex items-center gap-2 mb-1">
        <router-link to="/dashboard/products" class="text-[12px] font-bold text-slate-400 hover:text-emerald-500 transition-colors tracking-widest flex items-center gap-1">
          <i class="pi pi-shopping-bag text-[11px]"></i>
          {{ $t('menu.products') }}
        </router-link>
        <i class="pi pi-chevron-right text-[10px] text-slate-300"></i>
        <span class="text-[12px] font-bold text-slate-600 dark:text-slate-300 tracking-widest truncate">{{ product?.name || '...' }}</span>
      </div>
      ...
      <p class="text-[13px] text-slate-400 dark:text-slate-500 mt-0.5 font-medium truncate">{{ product?.category_name || $t('common.not_specified') }} • SKU: {{ product?.sku || product?.barcode || '—' }}</p>
    </div>
