<template>
  <div class="md:col-span-2 bg-white dark:bg-[#131d31] rounded-[18px] border border-slate-200 dark:border-transparent p-6 sm:p-8 flex flex-col shadow-sm">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
      <!-- Buy Price -->
      <div class="space-y-1">
        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ $t('products.detail.purchase_price') }}</span>
        <div class="flex items-baseline gap-2">
          ...
          <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ product?.currency_code }}</span>
        </div>
      </div>

      <!-- Sell Price -->
      <div class="space-y-1">
        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ $t('products.detail.sale_price') }} 💰</span>
        <div class="space-y-1">
          <div class="flex items-baseline gap-2">
            <span 
      ...
      <div class="pt-6 border-t border-slate-50 dark:border-slate-800">
        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ $t('products.detail.expected_margin') }}</span>
