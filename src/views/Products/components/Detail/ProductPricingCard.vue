<template>
  <div class="md:col-span-2 bg-white dark:bg-[#131d31] rounded-[18px] border border-slate-200 dark:border-transparent p-6 sm:p-8 flex flex-col shadow-sm">
    <h3 class="text-[12px] font-black text-slate-900 dark:text-white tracking-[0.2em] flex items-center gap-3 mb-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
      <!-- Buy Price -->
      <div class="space-y-1">
        <span class="text-[11px] font-black text-slate-400 tracking-widest">{{ $t('products.detail.purchase_price') }}</span>
        <div class="flex items-baseline gap-2">
          ...
          <span class="text-[12px] font-black text-slate-400 tracking-widest">{{ product?.currency_code }}</span>
        </div>
      </div>

      <!-- Sell Price -->
      <div class="space-y-1">
        <span class="text-[11px] font-black text-slate-400 tracking-widest">{{ $t('products.detail.sale_price') }} 💰</span>
        <div class="space-y-1">
          <div class="flex items-baseline gap-2">
            <span 
          ...
          <div v-if="product?.active_promotion" class="flex items-center gap-2">
            <span class="text-[11px] font-bold text-slate-400 line-through opacity-70">{{ formatPrice(product?.sale_price) }}</span>
            <span class="text-[10px] font-bold text-rose-500 bg-rose-500/10 px-1.5 py-0.5 rounded-md tracking-widest truncate max-w-[120px]">{{ product.active_promotion.name }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="pt-6 border-t border-slate-50 dark:border-slate-800">
        <span class="text-[11px] font-black text-slate-400 tracking-widest">{{ $t('products.detail.expected_margin') }}</span>
