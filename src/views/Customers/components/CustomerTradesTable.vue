  ...
  <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm h-full flex flex-col">
    ...
    <div class="px-1 border-b border-slate-100 dark:border-slate-800/50 bg-slate-50/50 dark:bg-slate-800/30">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between px-4 py-2.5 gap-3">
        <div class="flex items-center gap-2">
          ...
          <h3 class="text-[12px] font-black text-slate-700 dark:text-white tracking-widest truncate">{{ $t('customers.trades.title') }}</h3>
        </div>
        <div v-if="activeTab === 'debts' && totalDebt > 0" class="flex items-center gap-2 bg-rose-500/5 px-2 py-1 rounded-lg border border-rose-500/10 self-start sm:self-auto">
          <span class="text-[10px] font-black text-rose-500/70 tracking-widest leading-none">{{ $t('customers.details.debt') }}:</span>
          <span class="text-[11px] font-black text-rose-600 dark:text-rose-400 leading-none">
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <div class="relative group/search flex-grow sm:flex-grow-0">
              ...
              <input 
                ...
                class="bg-white dark:bg-slate-800 border border-slate-200/50 dark:border-slate-800/50 rounded-lg pl-7 pr-3 py-1.5 text-[11px] font-black tracking-wider text-slate-600 dark:text-slate-300 outline-none focus:border-emerald-500/50 w-full sm:w-32"
              />
            </div>
            ...
            <div class="flex items-center gap-0.5 bg-slate-100/50 dark:bg-slate-900/50 border border-slate-200/50 dark:border-slate-800/50 rounded-lg p-0.5 transition-colors flex-grow sm:flex-grow-0 justify-center">
              <div class="flex items-center px-1.5 py-1 gap-1 bg-white dark:bg-slate-800 rounded-md shadow-sm border border-slate-100 dark:border-slate-700 flex-1 sm:flex-none">
                ...
                <input type="number" v-model="filters.min_amount" @change="applyFilters" placeholder="0" class="bg-transparent border-none text-[11px] font-black text-slate-600 dark:text-slate-300 outline-none w-full sm:w-14" />
              </div>
              <div class="flex items-center px-1.5 py-1 gap-1 bg-white dark:bg-slate-800 rounded-md shadow-sm border border-slate-100 dark:border-slate-700 flex-1 sm:flex-none">
                <span class="text-[9px] font-black text-slate-400">MAX</span>
                <input type="number" v-model="filters.max_amount" @change="applyFilters" placeholder="∞" class="bg-transparent border-none text-[11px] font-black text-slate-600 dark:text-slate-300 outline-none w-full sm:w-14" />
              </div>
            </div>
            ...
            <div class="flex items-center gap-0.5 bg-slate-100/50 dark:bg-slate-900/50 border border-slate-200/50 dark:border-slate-800/50 rounded-lg p-0.5 transition-colors flex-grow lg:flex-grow-0 justify-center">
              <div class="flex items-center px-2 py-1 gap-1.5 bg-white dark:bg-slate-800 rounded-md shadow-sm border border-slate-100 dark:border-slate-700 flex-1 lg:flex-none">
                <i class="pi pi-calendar text-[11px] text-slate-400"></i>
                <input type="date" v-model="filters.date_from" @change="applyFilters" class="bg-transparent border-none text-[11px] font-black text-slate-600 dark:text-slate-300 outline-none w-full sm:w-auto " />
              </div>
              <div class="flex items-center px-2 py-1 gap-1.5 bg-white dark:bg-slate-800 rounded-md shadow-sm border border-slate-100 dark:border-slate-700 flex-1 lg:flex-none">
                <input type="date" v-model="filters.date_to" @change="applyFilters" class="bg-transparent border-none text-[11px] font-black text-slate-600 dark:text-slate-300 outline-none w-full sm:w-auto " />
                <i class="pi pi-calendar text-[11px] text-slate-400"></i>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
              <div class="relative group/select flex-grow sm:flex-grow-0">
                <select v-model="filters.payment_type" @change="applyFilters" 
                        class="appearance-none bg-white dark:bg-slate-800 border border-slate-200/50 dark:border-slate-800/50 rounded-lg pl-3 pr-7 py-1.5 text-[11px] font-black tracking-wider text-slate-600 dark:text-slate-300 outline-none focus:border-emerald-500/50 transition-all cursor-pointer w-full sm:w-auto">
              ...
              <button @click="toggleHasDebt" 
                      :class="['h-8 px-3 rounded-lg border text-[11px] font-black tracking-wider transition-all flex items-center justify-center gap-1.5 flex-grow sm:flex-grow-0', filters.has_debt ? 'bg-emerald-500 border-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-white dark:bg-slate-800 text-slate-500 border-slate-200/50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-700']">
      ...
      <div v-if="currentSummary" class="px-5 pb-4">
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/10 dark:border-emerald-500/20 rounded-2xl p-3 shadow-inner">
          <div class="flex flex-col">
            <span class="text-[10px] font-black text-emerald-500/70 tracking-widest">{{ $t('common.all') }}</span>
          ...
          <div class="flex flex-col border-l border-emerald-500/10 pl-3">
            <span class="text-[10px] font-black text-emerald-500/70 tracking-widest">{{ $t('common.net_price') }}</span>
            <span class="text-xs font-black text-slate-700 dark:text-slate-200">{{ formatCurrency(currentSummary.total_net) }}</span>
          </div>
          <div class="flex flex-col border-l border-emerald-500/10 pl-3">
            <span class="text-[10px] font-black text-rose-500/70 tracking-widest">{{ $t('common.debt_label') }}</span>
            <span class="text-xs font-black text-rose-500">{{ formatCurrency(currentSummary.total_debt) }}</span>
          </div>
          <div class="flex flex-col border-l border-emerald-500/10 pl-3">
            <span class="text-[10px] font-black text-emerald-500/70 tracking-widest">{{ $t('common.success') }}</span>
    ...
    <div class="flex-grow overflow-y-auto custom-scrollbar">
      <table class="w-full text-left border-separate border-spacing-y-2 px-4">
        <thead>
          <tr class="text-[11px] font-black tracking-[0.2em] text-slate-400/80">
        ...
        <tbody>
          <tr v-for="t in currentData" :key="t.id" 
            ...
            <td class="px-4 py-4 first:rounded-l-2xl">
              <div class="flex flex-col">
                <span class="text-[13px] font-black text-slate-900 dark:text-white leading-none whitespace-nowrap">
                  {{ t.created_on.split('|')[0] }}
                </span>
                <span class="text-[11px] font-black text-slate-400 mt-1 tracking-tighter">
                ...
                <div class="flex items-center gap-1.5 mt-2.5">
                  ...
                  <span class="text-[11px] font-black text-slate-500 dark:text-slate-400 truncate max-w-[80px]">{{ t.worker_name }}</span>
                </div>
              </div>
            </td>
            ...
            <td class="px-4 py-4 min-w-[200px]">
              <div class="flex flex-col gap-1.5">
                <div v-for="(item, idx) in t.items?.slice(0, 2)" :key="idx" class="flex items-center gap-2 group/item">
                  ...
                  <span class="text-[12px] font-black text-slate-700 dark:text-slate-200 flex items-center gap-1.5">
                ...
                <div v-if="t.items?.length > 2" class="text-[10px] font-black text-slate-400 tracking-widest mt-1">
            ...
            <td class="px-4 py-4">
              <div class="flex flex-col">
                ...
                <div v-if="Number(t.discount_amount) > 0" class="flex items-center gap-1 mt-1">
                  <span class="text-[11px] font-black text-rose-500 bg-rose-500/10 px-1.5 py-0.5 rounded-md">-{{ formatCurrency(t.discount_amount) }}</span>
                </div>
                <div v-if="Number(t.debt_amount) > 0" class="mt-1.5">
                  <span class="text-[11px] font-black text-amber-600 dark:text-amber-400 tracking-tighter bg-amber-500/10 px-2 py-0.5 rounded-full border border-amber-500/20">
            ...
            <td class="px-4 py-4 text-right last:rounded-r-2xl">
              <div class="flex flex-col items-end gap-2">
                ...
                <span v-if="t.branch_name" class="flex items-center gap-1 text-[10px] font-black text-slate-400 tracking-widest whitespace-nowrap">
    ...
    <div v-if="totalRecords > 0" 
      ...
      <div class="text-[12px] font-black text-slate-400 dark:text-slate-600 tracking-widest leading-none">
      ...
      <div class="flex items-center gap-1">
        <button @click="changePage(1)" :disabled="currentPage === 1"
          ...
          <i class="pi pi-angle-double-left text-[12px]"></i>
        </button>
        ...
        <button @click="changePage(totalPages)" :disabled="currentPage === totalPages"
                class="w-8 h-8 rounded-xl flex items-center justify-center border transition-all disabled:opacity-30
                       border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 shadow-sm">
          <i class="pi pi-angle-double-right text-[12px]"></i>
        </button>
      </div>
    </div>
