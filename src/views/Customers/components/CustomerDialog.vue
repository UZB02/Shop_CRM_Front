      ...
      <div v-if="visible" class="fixed top-0 right-0 h-full w-full max-w-[420px] bg-white dark:bg-slate-900 border-l border-slate-100 dark:border-slate-800 shadow-[-20px_0_50px_-12px_rgba(0,0,0,0.1)] z-[1000] flex flex-col">
        ...
        <div class="p-6 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between">
          <div class="flex flex-col">
            <span class="text-sm font-black uppercase tracking-widest text-emerald-500">{{ customer.id ? $t('customers.form.edit') : $t('customers.form.add') }}</span>
            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">{{ $t('customers.subtitle') }}</span>
          </div>
        ...
        <div class="p-6 space-y-6 flex-1 overflow-y-auto">
          <div class="space-y-4">
            <!-- Full Name -->
            <div class="field">
              <label for="name" class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 mb-1.5 block">{{ $t('customers.form.full_name') }} <span class="text-rose-500">*</span></label>
              ...
              <small class="p-error text-[9px] font-bold uppercase mt-1 ml-1" v-if="submitted && !customer.name">{{ $t('customers.form.name_required') }}</small>
            </div>

            <!-- Phone Number -->
            <div class="field">
              <label for="phone" class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 mb-1.5 block">{{ $t('customers.form.phone') }} <span class="text-rose-500">*</span></label>
              <div class="relative group/input">
              ...
              <small class="p-error text-[9px] font-bold uppercase mt-1 ml-1" v-if="submitted && !customer.phone">{{ $t('customers.form.phone_required') }}</small>
            </div>

            <!-- Customer Group -->
            <div class="field">
              <label for="group" class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 mb-1.5 block">{{ $t('customers.form.group') }}</label>
              <Select 
                id="group" 
            ...
            <!-- Address -->
            <div class="field">
              <label for="address" class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 mb-1.5 block">{{ $t('customers.form.address') }}</label>
              <div class="relative group/input">
