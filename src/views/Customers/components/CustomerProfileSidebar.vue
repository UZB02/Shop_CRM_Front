  ...
  <div class="bg-white dark:bg-slate-900/50 rounded-xl p-4 text-slate-900 dark:text-white shadow-sm border border-slate-100 dark:border-slate-800 relative overflow-hidden group h-fit">
    ...
    <div class="relative z-10 flex flex-col items-center">
      ...
      <div class="flex flex-wrap justify-center gap-1.5 mb-6">
        <span class="px-2 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 text-[7px] font-black tracking-widest text-emerald-600 dark:text-emerald-400 whitespace-nowrap">{{ customer.status_display || 'ACTIVE' }}</span>
        <span class="px-2 py-0.5 rounded-md bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 text-[7px] font-black tracking-widest text-slate-500 dark:text-slate-400 whitespace-nowrap">{{ customer.group_name }}</span>
      </div>
      
      <div class="w-full space-y-3 pt-4 border-t border-slate-100 dark:border-white/5">
        <div class="flex items-center justify-between gap-2">
            <span class="text-[7px] font-black text-slate-400 dark:text-slate-500 tracking-widest leading-none">{{ $t('customers.details.phone') }}</span>
            <span class="text-[10px] font-black text-slate-700 dark:text-slate-300 truncate">{{ customer.phone || '—' }}</span>
        </div>
        <div class="flex items-center justify-between gap-2">
            <span class="text-[7px] font-black text-slate-400 dark:text-slate-500 tracking-widest leading-none">{{ $t('customers.details.address') }}</span>
            <span class="text-[10px] font-black text-slate-700 dark:text-slate-300 truncate" :title="customer.address">{{ customer.address || '—' }}</span>
        </div>
        <div class="flex items-center justify-between gap-2">
            <span class="text-[7px] font-black text-slate-400 dark:text-slate-500 tracking-widest leading-none">{{ $t('customers.details.registered') }}</span>
      ...
      <button 
        ...
        @click="$emit('payDebt')"
        class="w-full mt-6 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-slate-900 text-[9px] font-black tracking-[0.15em] shadow-lg shadow-emerald-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
      >
