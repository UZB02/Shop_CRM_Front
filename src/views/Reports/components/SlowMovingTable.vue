<template>
  <div class="reports-table overflow-hidden">
    ...
    <div v-else-if="!items.length" class="flex flex-col items-center justify-center py-20 text-slate-400">
      <i class="pi pi-clock text-4xl mb-3 opacity-30"></i>
      ...
      <div class="hidden md:block overflow-x-auto">
        ...
        <thead>
          <tr class="border-b border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/40">
            ...
            <th class="px-4 py-3 text-right text-[11px] font-black text-slate-400 tracking-widest">{{ $t('reports.col_daily_sales') }}</th>
            <th class="px-4 py-3 text-right text-[11px] font-black text-slate-400 tracking-widest">{{ $t('reports.col_days_stock') }}</th>
            <th class="px-4 py-3 text-right text-[11px] font-black text-slate-400 tracking-widest">{{ $t('reports.col_idle_days') }}</th>
          </tr>
        </thead>
        <tbody>
          <tr
          ...
          >
            <!-- Rank -->
            <td class="px-4 py-3">
              <div class="w-7 h-7 rounded-lg bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 flex items-center justify-center text-[11px] font-black">
                {{ offset + idx + 1 }}
              </div>
            </td>
            ...
            <!-- Kunlik sotuv -->
            <td class="px-4 py-3 text-right">
              <span class="font-bold text-slate-600 dark:text-slate-300">{{ parseDecimal(item.avg_daily_sales).toFixed(2) }}</span>
            </td>

            <!-- Kunlar zaxirasi -->
            <td class="px-4 py-3 text-right">
              <span :class="daysStockColor(item.days_of_stock)" class="font-black text-sm">
                {{ item.days_of_stock ?? '—' }} kun
              </span>
            </td>

            <!-- Kutish -->
            <td class="px-4 py-3 text-right">
              <span class="text-[11px] font-bold text-amber-500 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-2 py-0.5 rounded-lg">
                {{ item.idle_days ?? '—' }} kun
              </span>
            </td>
          </tr>
      ...
      <div class="md:hidden flex flex-col gap-3 px-3 py-3">
        ...
        >
          ...
          <div class="flex items-start gap-3">
            <div class="w-8 h-8 rounded-xl bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 shrink-0 flex items-center justify-center text-xs font-black shadow-sm">
              {{ offset + idx + 1 }}
            </div>
          ...
          <div class="grid grid-cols-2 gap-y-3 gap-x-4">
            ...
            <div class="flex flex-col text-right">
              ...
              <span class="text-[10px] font-bold text-amber-500 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-1.5 py-0.5 rounded-lg inline-block w-max ml-auto">
                {{ item.idle_days ?? '—' }} kun
              </span>
            </div>
            
            <div class="flex flex-col">
              <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">{{ $t('reports.col_daily_sales') }}</span>
              <span class="font-bold text-slate-600 dark:text-slate-300 text-sm">{{ parseDecimal(item.avg_daily_sales).toFixed(2) }}</span>
            </div>
            <div class="flex flex-col text-right">
              <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">{{ $t('reports.col_days_stock') }}</span>
              <span :class="daysStockColor(item.days_of_stock)" class="font-black text-sm">
                {{ item.days_of_stock ?? '—' }} kun
              </span>
            </div>
...
import { formatTur, formatNum, parseDecimal } from '../composables/useProductReports'
...
function daysStockColor(days) {
  if (!days || days > 90) return 'text-rose-500 dark:text-rose-400'
