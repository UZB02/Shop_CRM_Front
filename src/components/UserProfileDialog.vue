<template>
  ...
  >
    <div v-if="user" class="flex flex-col bg-white dark:bg-[#0F172A] min-h-max overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-800/60 shadow-2xl shadow-slate-200/50 dark:shadow-black/60">
      
      <!-- Header: Compact & Professional -->
      <div class="relative h-28 bg-gradient-to-br from-[#10B981] to-[#059669] dark:from-[#2D7A5E] dark:to-[#1E4D3C] flex items-start justify-end p-4">
        <div class="absolute inset-0 bg-black/5 dark:bg-black/10"></div>
        <Button 
          icon="pi pi-times" 
          text 
          rounded 
          class="!text-white/90 hover:!text-white hover:bg-white/20 !w-8 !h-8 transition-all z-10" 
          @click="$emit('update:visible', false)" 
        />
        
        <div class="absolute -bottom-10 left-1/2 -translate-x-1/2">
          <div class="relative">
            <div class="w-24 h-24 rounded-full border-[6px] border-white dark:border-[#0F172A] bg-slate-50 dark:bg-slate-800 flex items-center justify-center shadow-xl overflow-hidden">
              <Avatar 
            ...
            <div class="absolute bottom-1 right-1 w-5 h-5 bg-[#22C55E] border-[3px] border-white dark:border-[#0F172A] rounded-full shadow-sm"></div>
          </div>
        </div>
      </div>
      ...
      <div class="pt-12 px-6 pb-6">
        <div class="text-center mb-6">
          ...
          <div class="flex items-center justify-center gap-2 mt-1.5">
            <span class="px-2 py-0.5 bg-emerald-50 dark:bg-[#064E3B] text-emerald-600 dark:text-[#10B981] text-[9px] font-black uppercase rounded tracking-widest border border-emerald-500/20">
              {{ user.worker?.role_display || 'USER' }}
            </span>
            <span class="text-slate-400 dark:text-slate-500 text-[10px] font-medium">• ID: #{{ user.id }}</span>
          </div>
        </div>
        ...
        <!-- Branch Snapshot -->
        <div class="mt-6 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-700/50 flex items-center gap-3">
          <div class="w-10 h-10 rounded-lg bg-white dark:bg-slate-800 flex items-center justify-center text-emerald-500 shadow-sm dark:shadow-inner border border-slate-100 dark:border-slate-700">
          ...
          <div class="flex flex-col overflow-hidden">
            ...
            <span class="text-sm text-slate-800 dark:text-white font-bold truncate">{{ user.worker?.store_name }}</span>
        ...
        <div class="mt-6 pt-4 flex items-center gap-3">
          <Button 
            ...
            severity="danger" 
            text
            class="flex-1 font-bold text-xs bg-rose-50 dark:bg-rose-500/5 hover:bg-rose-100 dark:hover:bg-rose-500/10 text-rose-500 border border-rose-200 dark:border-rose-500/10 h-10 transition-all rounded-xl"
            @click="$emit('logout')" 
          />
          <Button 
            label="Yopish" 
            text 
            class="flex-1 font-bold text-xs text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:text-white h-10 transition-all border border-slate-200 dark:border-slate-800 rounded-xl" 
            @click="$emit('update:visible', false)" 
          />
        </div>
      </div>
